...
def get_api_token():
    try:
        response = requests.post(
            LOG_IN_URL, headers=LOGIN_HEADERS, data=json_auth_params
        )
...
auth_params = {"email": os.getenv("API_USER"), "password": os.getenv("API_PASSWORD")}
...
sprite_sheet, palette = adafruit_imageload.load(
    "/trend_arrows.bmp", bitmap=displayio.Bitmap, palette=displayio.Palette
)

trend_arrows = displayio.TileGrid(
    sprite_sheet, pixel_shader=palette, width=1, height=1, tile_width=16, tile_height=16
)
